...
namespace MonsterSystem
{
	...
	internal class MonsterIsNearToChase : INode
	{
		...
		{
			targetPosition = new Vector3(targetPosition.x, monsterPosition.y, targetPosition.z); // y축(상하)은 고려하지 않습니다.
			float calculatedDistance = Vector3.Distance(monsterPosition, targetPosition); // 몬스터와 목표 사이의 거리를 계산합니다.
		...
		{
			NodeState nodeState = (distance <= chaseRange) ? NodeState.Success : NodeState.Failure; // 추적 범위 안에 있다면 성공을, 없다면 실패를 반환합니다.
