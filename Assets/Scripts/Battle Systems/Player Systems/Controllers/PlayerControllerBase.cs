...
namespace PlayerSystem
{
	...
	internal class PlayerControllerBase : MonoBehaviour, IPlayerInput
	{
		...
		// [변수] 플레이어(발키리)의 데이터
		internal PlayerBattleData BattleData { get; private set; }

		// [변수] 상태 패턴에 사용하는 변수들
		private Dictionary<Type, PlayerStateBase> _stateCache; // 상태 클래스 목록을 캐시화합니다.
		private IState _currentState; // 플레이어의 상태를 지정 및 관리하기 위한 인터페이스
		...
		internal void InitializeData(PlayerBattleData data)
		{
			BattleData = data;
		}
