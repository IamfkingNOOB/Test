...
namespace PlayerSystem
{
	...
	/// </summary>
	internal class PlayerPetSkillState : PlayerStateBase
	{
		#region 변수(필드)

		// [변수] 애니메이터(Animator)의 컴포넌트 및 매개변수
		private readonly Animator _animator; // 애니메이터의 컴포넌트
		private readonly int _petSkillAnimatorHash = Animator.StringToHash("petSkill"); // 애니메이터의 매개변수 해시 (Bool)
		...
		// [생성자] 변수를 초기화합니다.
		internal PlayerPetSkillState(PlayerControllerBase controller) : base(controller)
		{
		...
		{
			_animator.SetTrigger(_petSkillAnimatorHash);
		}

		// [함수] 애니메이션을 정지합니다.
		private void StopAnimation()
		{
			_animator.ResetTrigger(_petSkillAnimatorHash);
		}
